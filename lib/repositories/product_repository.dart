...
import 'package:union/data_model/product_mini_response.dart';
import 'package:union/data_model/product_details_response.dart';
import 'package:union/data_model/variant_response.dart';
...
class ProductRepository {
  ...
  Future<ProductMiniResponse> getBundleProducts(
      {@required int id = 0}) async {
    Uri url =
    Uri.parse("${AppConfig.BASE_URL}/bundle-products/" + id.toString());
    final response = await http.get(url, headers: {
      "App-Language": app_language.$,
    });
    return productMiniResponseFromJson(response.body);
